...
age = st.number_input("enter Age", min_value=0, max_value=120, value=0)
sex = st.selectbox("Sex", ["M", "F"])
chest_pain = st.selectbox("Chest Pain Type", ["ATA", "NAP", "ASY", "TA"])
resting_bp = st.number_input("Resting Blood Pressure", min_value=0, max_value=200, value=0)
...
max_hr = st.number_input("Max Heart Rate", min_value=0, max_value=220, value=0)
exercise_angina = st.selectbox("Exercise Angina", ["N", "Y"])
oldpeak = st.number_input("Oldpeak", min_value=0.0, max_value=6.0, value=0.0)
...
if st.button('Predict'):
    ...
    if prediction[0] == 1:
      st.error("Prediction: Heart Disease Present") # error - RED color
    else:
      st.success("Prediction: No Heart Disease")  # success - GREEN color
